/**
 * Vite plugin for @ducklings/workers-ducklake on Cloudflare Workers
 *
...
/**
 * Resolves the path to the WASM file bundled with @ducklings/workers-ducklake.
 *
 ...
 *
 * This plugin:
 * - Resolves `@ducklings/workers-ducklake/wasm` imports to a relative path for wrangler
...
export function ducklingsWorkerPlugin(options: DucklingsWorkersPluginOptions = {}): VitePlugin {
  ...
  return {
    name: 'ducklings-workers-ducklake-plugin',
    ...
    resolveId(id) {
      if (id === '@ducklings/workers-ducklake/wasm') {
